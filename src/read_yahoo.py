...
def pull_last_year_draft(league_id_previous, save_dir, cookies_dict):
...
def pull_last_year_dropped(league_id_previous, save_dir, cookies_dict):
    previous_year = datetime.now().year - 1
...
def pull_last_season_rosters(league_id_current, save_dir, cookies_dict):
...
def pull_traded_picks(league_id, save_dir, cookies_dict):
...
def pull_projected_points(league_id, save_dir, cookies_dict, max_players=300):
    base_url = f"https://football.fantasysports.yahoo.com/f1/{league_id}/players?&pos=O&sort=OR&sdir=1&status=ALL&eteam=ALL&fteam=NONE&stat1=S_PS_2026&count={{}}"
    rows = []
    all_players = set()
